  ...
  try {
    ...
    if (API) {
      const res = await fetch(
        `${API}/anime/episode-srcs?id=${episodeid}&server=vidstreaming&category=${subtype}`
      );
      data = await res.json();
    } else {
...
export async function getHianimeSources(id) {
  try {
...
export async function getAnimeSources(id, provider, epid, epnum, subtype) {
  try {
    ...
    if (provider === "hianime") {
      const data = await getHianimeSources(id);
      return data;
    }
  } catch (error) {
    console.log(error);
